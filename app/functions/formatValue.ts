export const formatValue = (inputValue:string, format:string, currencySymbol:string, digits:number, abbreviate:string)=>{
  ...
  export const getValue =  (inputValue:string, format:string)=>{
  
    let stringValue = inputValue.toString().toLowerCase()
    let numericalValue = Number(stringValue.replace(/[^0-9.]/g,'')); 
    let outputValue = 0
    let exponent = 1
  ...
  //   Adjust for percentage formatting
     ...
     return outputValue = numericalValue/100;
    }
